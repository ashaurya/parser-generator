...
#print "<tr>"
#print "<td width=\"40%\" align=\"center\"><b>Non-Terminals</b></td>"
#print "<td width=\"60%\" align=\"center\"><b>Terminals in First set</b></td>"
#print "</tr>"
for r in code.rules:
	print "<tr>"
...
#print "hello",place_sym(nonterm_initial2)
nonterm_initial2_place_sym=place_sym(nonterm_initial2)
#print "hello", nonterm_initial2_place_sym
...
#for a in code.nonterms:
	#print "first set for",a,"=",firstNt[i]
	#i = i+1
...
#			print t
#			print "<BR>"
#			if ptable[place_sym(i)][t] is not None:
...
#		print "<td width=\"350\" align=\"center\">",ptable[place_sym(i)],"</td>"	
		print "</tr>"	
	print "</table><BR>"
	print "</div>"
...
#  print	
############################-------------------ITEM SET PRINTING
############################################ Check it out for goto and item sets
goto=[]
p=0
...
while lr0_actionCount<len(I):
  ...
  for a in code.terminals:
    ...
    p=0
    if len(I_1)==0:
      #lr0_action[lr0_actionCount][count].append(' ')
      count+=1
    else:
      for p in range(0,len(I)):
	if I[p][0]==I_1:
  ...
  for index in I[lr0_actionCount-1][0]:
	...
	ruleNo+=1
      if ruleFlag:
	#print lr0_actionCount-1,ruleNo,rule
	A=index[0]
	count=1
	for a in code.terminals:
	  if not A==nonterm_initial:
	    if a=='$': #and len(I[lr0_actionCount-1][0])==1:
...
for a in range(0,len(lr0_action[p+1])):
  if lr0_action[0][a]=='$':
    #lr0_action[p+1][a]="acc"
    lr0_action[p+1][a].append("acc")
    break
#print len(lr0_action[1][2])
############################ end code for accept
...
while actionCount<len(I):
  ...
  for a in code.terminals:
    ...
    p=0
    if len(I_1)==0:
      #action[actionCount][count].append(' ')
      count+=1
    else:
      for p in range(0,len(I)):
	if I[p][0]==I_1:
  ...
  for index in I[actionCount-1][0]:
	...
	for a in code.terminals:
	  if a in followNt[place_sym(A)] and not A==nonterm_initial:
	    if a=='$': #and len(I[actionCount-1][0])==1:
...
#-------------------------------LR(1)____________________________________________________________


###################################-------------------------LR(1) items code from here
#CLOSURE(I)
#{
#repeat
...
#			print "ITEMS: \n",_item_,"<BR>"
			...
			if(place >= len(_item_)-2 or place==0):
				break
			else: ######################---------epsilon case remaining!!!!!!!!!!!!!!!!!!!!!---recursion!!!!
				if(_item_[place] in code.nonterms):
					for a in code.rules:
						if(_item_[place]==a[0]):
							if(_item_[place] in code.nonterms):
#								print "Item: ",_item_,"Item[place]: ",_item_[place+1],"<BR>\n"
#								print "item ",_item_,"<BR>"	
...
#									if(temp[0]=='E'):
#										print "1 ",temp,"<BR>","ITEM[place+1]: ",_item_[place+1]
#									print "temp1:   ",temp
#									print "<BR>"
...
#									change= 1
##								continue
#							if((_item_[place+1] in code.terminals) and place!=len(_item_)-3):
#								temp = []
#								temp.extend(a)
#								temp.extend('1')
#								temp.extend(_item_[place+1])
...
#									change= 1
##								continue	
#							for f in firstNt[place_sym(_item_[place+1])]:
#								if f in code.terminals:
#									temp = []
#									temp.extend(a)
#									temp.extend('1')
#									temp.append(f)
...
#LR_I_o=copy.deepcopy(I_o)
#for a in range(0,len(LR_I_o)):
#	LR_I_o[a].append('$')
...
#AILA=LR_calc_goto(CLOSURE,'c')
#print "-----------AIIIIIIIIIILLLLLLLLLLLLLLAAAAAAA\n"
#for s in AILA:
#	print s
#	print
#	print	
...
###############-------------THE BUG IS HERE----------------DUPLICATE ITEM SETS!!!!!!!!!!!---- 
def same(a,b):
	lena=len(a)
	lenb=len(b)
	same_or_not=False
...
#for r in range(0,len(LR_I)):
#	LR_I[r]=sorted(LR_I[r])
#for r in  range(0,len(LR_I)):
#	LR_I[r][0]=list(set(LR_I[r][0]))
#copies=[]
#for r in range(0,len(LR_I[0])):
#	for s in range(r+1,len(LR_I[0])):
#		if(len(LR_I[0][s])==len(LR_I[0][r])):
##			print "SAME!!!!<BR>"
#			number=0
#			print "LR_I: <BR>"
#			print LR_I[0][s],
#			print " , ",len(LR_I[0][s])
#			print "END <BR>"
#			for x in range(0,len(LR_I[0][r])):
#				for y in range(0,len(LR_I[0][r])):
#					if(LR_I[0][s][x]==LR_I[0][r][y]):
#						number+=1
#			if(number==len(LR_I[0][s])):
#				copies.append(s)
#for r in copies:
#	print "copies of : ",r,"<BR>"
#	LR_I[0].remove(LR_I[0][r])				
...
#				for r in m:
#					for s in n:
#						if (collections.Counter(s)!=collections.Counter(r)):
#							if(len(r)==len(s)):
#	#							print "SAME!!!!<BR>"
#								number=0
##								print "LR_I: <BR>"
		...
		if(len_ori==len_cop):
			...
			for k in range(0,len_ori):
				for l in range(0,len_ori):
					u=len(LR_I[i][0][k])
					v=len(LR_I[j][0][l])
#					print "LR_I[i][0][k]: ",LR_I[i][0][k],"<BR>"
#					print "LR_I[j][0][l]: ",LR_I[j][0][l],"<BR>"
					if(u==v):
...
##################################--------------------------------LR(1) items code finishes here




##############_____________________________PRINTING LR(1) ITEM SETS!!!!!!!!!!!!!!!!!!!
...
############_________________________ACTION TABLE CODE FOR LR(1)
...
while LRactionCount<len(LR_I):
  ...
  for a in code.terminals:
    ...
    p=0
    if len(LR_I_1)==0:
      #LR_action[LRactionCount][count].append(' ')
      count+=1
    else:
      for p in range(0,len(LR_I)):
...
#	for a in code.terminals:
#	  if a in followNt[place_sym(A)] and not A==nonterm_initial:
#	    if a=='$': #and len(LR_I[LRactionCount-1][0])==1:
#	      LR_action[LRactionCount][count].append('r'+str(ruleNo))
#	    elif not a=='$':
#	      LR_action[LRactionCount][count].append('r'+str(ruleNo))
#	  count+=1
	
#  for a in LR_I[LRactionCount-1]:
...
#					print "LR_I[p][r][k]: \n"
#					print LR_I[p][r][k],p,r,k,LR_I[p][r][k][0],tcount
#					print "\nHELLO"
					number=0
					string="5"
					for R in code.rules:
...
############_________________________ACTION TABLE CODE FOR LR(1) ends here

#########____________________________________LLLLLLLLLLLLLLAAAAAAAAAAALLLLLLLLLLRRRRRRRRRRRRRRR CODE STARTS HERE-----------------------------------------------------------------------------------------
...
#LA_I_o=copy.deepcopy(I_o)
#for a in range(0,len(LA_I_o)):
#	LA_I_o[a].append('$')
...
#------------------------------merging item sets which are with same cores
############-------------------list of merged item sets as per LR(1) item sets and LALR item sets
merged=[]
...
################____________________________________________FINAL MERGING!!!
...
##################################--------------------------------LALR  items code finishes here




##############_____________________________PRINTING LALR ITEM SETS!!!!!!!!!!!!!!!!!!!
...
############________________________________________ACTION TABLE CODE FOR LALR
...
while mLAactionCount<len(mLA_I):
  ...
  for a in code.terminals:
    ...
    p=0
    if len(mLA_I_1)==0:
      #mLA_action[mLAactionCount][count].append(' ')
      count+=1
    else:
      for p in range(0,len(mLA_I)):
...
#	for a in code.terminals:
#	  if a in followNt[place_sym(A)] and not A==nonterm_initial:
#	    if a=='$': #and len(mLA_I[mLAactionCount-1][0])==1:
#	      mLA_action[mLAactionCount][count].append('r'+str(ruleNo))
#	    elif not a=='$':
#	      mLA_action[mLAactionCount][count].append('r'+str(ruleNo))
#	  count+=1
	
#  for a in mLA_I[mLAactionCount-1]:
...
for a in range(0,len(mLA_action[p+1])):
  if mLA_action[0][a]=='$':
    #mLA_action[p+1][a]="acc"
    mLA_action[p+1][a].append("acc")
    break
#print len(mLA_action[1][2])
############################ end code for accept
...
#					print "mLA_I[p][r][k]: \n"
#					print mLA_I[p][r][k],p,r,k,mLA_I[p][r][k][0],tcount
#					print "\nHELLO"
					number=0
					string="empty"
					for R in code.rules:
...
#						print R
						...
						number+=1
#					if(len(mLA_action[p+1][tcount])==0):		
#						mLA_action[p+1][tcount].append(string)
...
############_________________________ACTION TABLE CODE FOR LALR ends here


	
		
###################################################### main.py code ends here
